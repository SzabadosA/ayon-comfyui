...
class AyonPublisher:
    ...
    def _construct_publish_path(
        ...
        The ``task_name`` parameter is injected into template data.  The
        resulting template string used for path formatting is returned along
        with the final publish path.
        """
        ...
        try:
            ...
            template_string = os.path.normpath(
                os.path.join(directory_template, file_template)
            )
            ...
            return publish_path, template_string
        except Exception as e:
    ...
    def _create_representation(
        ...
        Args include ``task_name`` to set context information. The ``template``
        argument should contain the formatted template string returned by
        :func:`_construct_publish_path`.
        """
    ...
    def _publish_sequence(
        ...
        for file_path in files:
            ...
            publish_path, template_string = self._construct_publish_path(
                file_path=file_path,
                project_name=project_name,
                folder_path=folder_path,
                task_name=task_name,
